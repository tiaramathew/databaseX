...
export class MongoDBAdapter implements VectorDBAdapter {
    ...
    async connect(config: ConnectionConfig): Promise<void> {
        ...
        // Instead, we'll make API calls to our backend which handles the connection
    ...
    async testConnection(): Promise<TestConnectionResult> {
        ...
        try {
            const response = await fetch("/api/db/test", {
                ...
                body: JSON.stringify({ type: "mongodb_atlas", config: this.config }),
            });
            if (response.ok) {
    ...
    async updateCollection(name: string, updates: UpdateCollectionConfig): Promise<void> {
        // MongoDB collections don't need schema updates typically
    }
